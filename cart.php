...
<div class="container">
	...
	<div class="row">
    <div class="col-md-12 show" style="margin-bottom:8%" id="content-body">
		<div>商品列表</div><br />
        <div>
		<table class="table table-bordered carttable" >
        <tr>
        <th class="cartgoodstatle">商品名稱</th>
        <th class="cartsmalltatle">單價</th>
        <th class="cartsmalltatle">數量</th>
        <th class="cartsmalltatle">小計</th>
        <th class="cartsmalltatle">刪除</th>
        </tr>
...
for($i = 0 ; $i < $value ; $i++)
{
	...
	$IncartPrice = $_SESSION[$user][$i]['price'];
	$IncartId =$_SESSION[$user][$i]['id'];
	$IncartType = $_SESSION[$user][$i]['type'];
		?>
        <form name="delcart" action="delcart.php" method="get"> 
        <input type="hidden" name="i" value="<?php echo $i?>" />
        <input type="hidden" name="user" value="<?php echo $user?>" />
        <tr>
        <td class="text-left">
        <div style="float:left;margin-right:20px;">
        <a href="<?php echo $IncartType;?>.php?no=<?php echo $IncartId;?>">
        <img src="image/<?php echo $IncartType;?>/<?php echo $IncartId;?>/1.jpg" class="img-rounded" style="height:100px; width:100px">
        </a></div>
		<div style="float:left;" >
        <a href="<?php echo $IncartType;?>.php?no=<?php echo $IncartId;?>">
		<?php echo $IncartProduct." (".$IncartColor.") "." (".$IncartSize.") </a></br>"."編號：".$IncartIdnum;?>
        </a>
        </div>
        </td>
        <td class="text-right">
        <div class="centerIntro"><?php echo $IncartPrice;?></div></td>
        <td class="text-right">
		<div class="centerIntro"><?php echo $IncartQuantity;?></div></td>
        <td class="text-right">
		<div class="centerIntro"><?php $total = $IncartPrice*$IncartQuantity;  echo number_format($total); ?></div></td>
        <td class="text-center">
        <div class="centerIntro"><input type="image" title="刪除" src="image/delet-trashcan.gif" onClick="document.delcart.submit();"></div></td>
        </tr>
        </form>
<?php 
$AllTotal += $total; 
...
?>
        <tr>
        <td style="	text-align:right" colspan=3>商品總計</td>
        <td style="	text-align:right" colspan=2>$<?php echo number_format($AllTotal); ?></td>
        </tr>
        </table>
    	</div>
        <div class="alert  alert-info">填寫訂購人資料</div>
        <div><b>輸入訂購人資料</b> >> 結帳</div><br />
        <div>
        <form action="order.php" method="post" onSubmit="return checkSendData(this)" name="formcheck" >
        <input type="hidden" name="total" value="<?php echo number_format($AllTotal)?>" />
        <table class="table">
        <tr>
        <td class="cartprice">付款方式</td>
        <td><input name="paytype" type="radio" value="ATM" checked="checked" />ATM匯款</td>
        </tr>
        <tr>
        <td class="cartprice">中文全名</td>
        <td><input name="name" id="name"/><span style="color:#F00">＊</span></td>
        </tr>
         <tr>
        <td class="cartprice">郵遞區號</td>
        <td>
...
<option value="983 花蓮縣 富里鄉">983 花蓮縣 富里鄉</option>
        ...
        <tr>
        <td class="cartprice">送貨地址</td>
        <td><input name="address" id="address" size="50" maxlength="50" /><span style="color:#F00">＊</span></td>
        </tr>    
        <tr>
        <td class="cartprice">手機號碼</td>
        <td><input name="phonenum" id="phonenum" size="10" maxlength="10" /><span style="color:#F00">＊</span></td>
        </tr>
        <tr>
        <td></td>
